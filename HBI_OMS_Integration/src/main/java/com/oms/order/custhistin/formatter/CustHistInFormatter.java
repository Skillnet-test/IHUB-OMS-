...
import com.oms.order.custhistin.formatter.schema.CustHistInMessage;
import com.oms.order.invoiceout.formatter.schema.InvoiceOutMessage;
import com.oms.order.orderout.formatter.schema.OrderOutMessage;
import com.oms.order.orderout.formatter.schema.OrderOutMessage.Header;
import com.oms.order.orderout.formatter.schema.OrderOutMessage.Header.Payments;
import com.oms.order.orderout.formatter.schema.OrderOutMessage.Header.Payments.Payment;

/**
 * @author Jigar
 */
@Component
public class CustHistInFormatter
{

    private static final Logger logger = Logger.getLogger(CustHistInFormatter.class);

    public String prepareRequest(CustHistInRequest omsCustHistRequest) throws OMSException
    {
        ...
        {
            logger.error(ex);
            throw new OMSException(OMSErrorCodes.REQUEST_PARSE_ERROR.getCode(),
                    OMSErrorCodes.REQUEST_PARSE_ERROR.getDescription());
        }
        return requestStr;
    }

    public CustHistInResponse translateResponse(String response) throws OMSException
    {
    	CustHistInResponse omsCustHistResponse = new CustHistInResponse();
        try
        {
    ...
    public OMSOrderResponse formatInvoiceOutToResponseObject(String invoiceOutResponseStr)
    {
        ...
        try
        {
            if (StringUtils.isNotEmpty(invoiceOutResponseStr))
            {
                ...
                String invoiceOutResponseStatus = invoiceOutJSONObject
                ...
                if (invoiceOutResponseStatus.equalsIgnoreCase(OrderConstantIfc.INVOICE_OUT_RESPONSE_STATUS_OK))
                {
                    ...
                    InvoiceOutMessage invoiceOutMessage = mapper.readValue(
