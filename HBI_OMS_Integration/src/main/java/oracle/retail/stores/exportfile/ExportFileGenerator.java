...
import oracle.retail.stores.exportfile.rtlog.RTLogClearTextOutputAdapter;
import oracle.retail.stores.exportfile.rtlog.RTLogOutputAdapter;
...
public class ExportFileGenerator implements ExportFileGeneratorIfc {
    ...
    public void process(WorkUnitIfc workUnit, String storeID) throws WorkUnitControllerException 
    {
        if (this.initFailed) {
            String message = "Export file initialization failed;  no records will be exported.  Please check the log for other initialization error messages.";
        ...
        try 
        {
            ((RTLogClearTextOutputAdapter)this.outputAdapter).setStoreID(storeID);
            this.outputAdapter.open();
            this.resultLogger.setExportFileName(this.outputAdapter.getFileName());
        } 
        catch (ExportFileException efe) 
    ...
    public void setFirstInBatch(boolean firstInBatch)
    {
